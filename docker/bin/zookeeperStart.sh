...
if [[ "$WRITE_CONFIGURATION" == true ]]; then
  ...
  if [[ $MYID -eq 1 ]]; then
    ...
    CONFIG=`java -Dlog4j.configuration=file:"$LOG4J_CONF" -jar /data/zu.jar get-all $ZKURL`
...
if [[ "$REGISTER_NODE" == true ]]; then
    ...
    java -Dlog4j.configuration=file:"$LOG4J_CONF" -jar /data/zu.jar add $ZKURL $MYID  $ZKCONFIG $DYNCONFIG
    set +e
fi
